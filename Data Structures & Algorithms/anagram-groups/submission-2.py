class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for word in strs:
            key = "".join(sorted(word)) # join the sorted characters back into a string
            result[key].append(word) # get the values list for this key, then add this word to it
        return list(result.values())

        # Time: O(m * nlog n) m strings, each sorted in n log n time
        # Space: O(m * n) m strings, each up to n characters long